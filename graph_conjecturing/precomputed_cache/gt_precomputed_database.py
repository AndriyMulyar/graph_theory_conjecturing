...
def precomputed_invariants_for_conjecture(database=None):
    ...
    """
    return (invariants_as_dict(database), (lambda g: g.canonical_label(algorithm='sage').graph6_string()),
            (lambda f: f.__name__))
...
def precomputed_properties_for_conjecture(database=None):
    ...
    default to the default database of get_connection().
    """
    return (properties_as_dict(database), (lambda g: g.canonical_label(algorithm='sage').graph6_string()),
            (lambda f: f.__name__))
    ...
    if not overwrite:
        if result is not None:
            ...
            if value != stored_value and abs(value - stored_value) > epsilon:
                print "Stored value of {} for {} differs from provided value: {} vs. {}".format(i_key, graph.name(),
                                                                                                stored_value, value)
            elif verbose:
            ...
            if g_key in current:
                if inv.__name__ in current[g_key]:
                    ...
                    try:
                        ...
                        cancel_alarm()
                        if result:
                            if result != current[g_key][inv.__name__] and abs(
                                    value - current[g_key][inv.__name__]) > epsilon:
        ...
        for idx, g in enumerate(graphs):
            ...
            if g_key in current:
                if prop.__name__ in current[g_key]:
                    print('Skipping: %s' % g_key)
                    continue

            result = None
            try:
                alarm(timeout)
                print('Computing %i / %i %s' % (idx, len(graphs), g_key))
                ...
                cancel_alarm()
                if result != None:
                    conn.execute("INSERT INTO prop_values(property, graph, value) VALUES (?,?,?)",
                                 (prop.__name__, g_key, result))
                    # commit the data so we don't lose anything if we abort early
                    conn.commit()
                else:
                    # the computation might have crashed
...
def store_property_value(property, graph, value, overwrite=False, database=None, verbose=False):
    ...
    if not overwrite:
        if result is not None:
            ...
            if value != stored_value:
                print "Stored value of {} for {} differs from provided value: {} vs. {}".format(p_key, graph.name(),
                                                                                                stored_value, value)
            elif verbose:
    ...
    if result is None:
        ...
        conn.execute("UPDATE prop_values SET value=(?) WHERE property = (?) AND graph = (?)",
                     (bool(value), p_key, g_key))
    conn.commit()
    conn.close()
    if verbose:
            ...
            if g_key in current:
                if prop.__name__ in current[g_key]:
                    ...
                    try:
                        ...
                        # Computation did not end. We interrupt/kill it.
                        print "Computation of {} for {} did not end in time... killing!".format(prop.__name__, g_key)
                    else:
                        # computation did end, so we verify the value
                        cancel_alarm()
                        if result is not None:
                            ...
                            # the computation might have crashed
                            print "Computation of {} for {} failed!".format(prop.__name__, g_key)
...
def precomputed_graphs_by_properties(graphs, properties, database=None):
    ...
    fully_precomputed_graphs = []
    for g in graphs:
        #print(len(fully_precomputed_graphs))
        g_key = g.canonical_label(algorithm='sage').graph6_string()
...
def precomputed_only_property_conjecture(objects, properties, mainProperty, precomputed_db=None,
                                         time=5, debug=False, verbosity=0, sufficient=True, operators=None,
                                         theory=None):
    """
    ...
    if verbosity >= 1:
        print clock.asctime(clock.localtime(clock.time())) + " Filtering finished. " + str(
            len(fully_precomputed_graphs)) + " graphs remaining for conjecture."
        print clock.asctime(clock.localtime(clock.time())) + " Conjecturing start."
    conjectures = propertyBasedConjecture(fully_precomputed_graphs, properties, mainProperty, time=time, debug=debug,
                                          verbose=(verbosity >= 2),
...
def precomputed_only_invariant_conjecture(objects, invariants, mainInvariant, precomputed_db=None,
    ...
    if verbosity >= 1:
        print clock.asctime(clock.localtime(clock.time())) + " Filtering finished. " + str(
            len(fully_precomputed_graphs)) + " graphs remaining for conjecture."
        print clock.asctime(clock.localtime(clock.time())) + " Conjecturing start."
    conjectures = conjecture(fully_precomputed_graphs, invariants, mainInvariant, variableName=variableName, time=time,
                             debug=debug,
